...
const createAgency = async (req, res) => {
    ...
    try {
        ...
        if (!subscription_end_date) return res.status(400).json({error: 'نوع اشتراک نامعتبر است.'});

        const agencyResult = await client.query(
            'INSERT INTO agencies (name, address, status, subscription_end_date, subscription_type) VALUES ($1, $2, $3, $4, $5) RETURNING id',
            [name, address, 'active', subscription_end_date, subscription_type]
        );
        ...
        await client.query(
            ...
            [admin_username, hash, 'agency_admin', newAgencyId]
        );
...
const updateAgency = async (req, res) => {
    ...
    try {
        ...
        let new_subscription_type = agency.rows[0].subscription_type;
        ...
        await client.query(
            ...
            [name, address, status, new_subscription_type, new_subscription_end_date, req.params.id]
        );
...
const changeUserPassword = async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query('UPDATE users SET password = $1 WHERE id = $2', [hash, req.params.userId]);
...
const changeSuperAdminCredentials = async (req, res) => {
    ...
    try {
        ...
        if (!match) return res.status(401).json({ error: 'رمز فعلی اشتباه است' });
        
        const hash = await bcrypt.hash(newPassword, 10);
        await pool.query('UPDATE users SET username = $1, password = $2 WHERE id = $3', [newUsername, hash, user.id]);
...
const createSupportAdmin = async (req, res) => {
    ...
    try {
        ...
        await pool.query("INSERT INTO users (username, password, role, agency_id) VALUES ($1, $2, 'support_admin', 1)", [username, hash]);
